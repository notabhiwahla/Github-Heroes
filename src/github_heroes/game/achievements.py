...
def check_achievements(player: Player, context: Optional[Dict] = None) -> List[str]:
    ...
    if player.level >= 10 and not AchievementRepository.has_achievement(player.id, "level_10"):
        unlock_achievement(player.id, "level_10")
        newly_unlocked.append("level_10")
    if player.level >= 25 and not AchievementRepository.has_achievement(player.id, "level_25"):
        unlock_achievement(player.id, "level_25")
        newly_unlocked.append("level_25")
    if player.level >= 50 and not AchievementRepository.has_achievement(player.id, "level_50"):
        unlock_achievement(player.id, "level_50")
        newly_unlocked.append("level_50")
    if player.level >= 100 and not AchievementRepository.has_achievement(player.id, "level_100"):
        unlock_achievement(player.id, "level_100")
        newly_unlocked.append("level_100")
    
    # Combat achievements
    if stats["enemies_defeated"] >= 1 and not AchievementRepository.has_achievement(player.id, "first_blood"):
        unlock_achievement(player.id, "first_blood")
        newly_unlocked.append("first_blood")
    if stats["enemies_defeated"] >= 50 and not AchievementRepository.has_achievement(player.id, "elite_hunter"):
        unlock_achievement(player.id, "elite_hunter")
        newly_unlocked.append("elite_hunter")
    if stats["enemies_defeated"] >= 100 and not AchievementRepository.has_achievement(player.id, "unstoppable"):
        unlock_achievement(player.id, "unstoppable")
        newly_unlocked.append("unstoppable")
    if stats["bosses_defeated"] >= 10 and not AchievementRepository.has_achievement(player.id, "boss_slayer"):
        unlock_achievement(player.id, "boss_slayer")
        newly_unlocked.append("boss_slayer")
    if stats["perfect_victories"] >= 1 and not AchievementRepository.has_achievement(player.id, "perfect_victory"):
    ...
    if world_count >= 5 and not AchievementRepository.has_achievement(player.id, "world_explorer"):
        unlock_achievement(player.id, "world_explorer")
        newly_unlocked.append("world_explorer")
    if world_count >= 25 and not AchievementRepository.has_achievement(player.id, "globe_trotter"):
        unlock_achievement(player.id, "globe_trotter")
        newly_unlocked.append("globe_trotter")
    if world_count >= 100 and not AchievementRepository.has_achievement(player.id, "cartographer"):
        unlock_achievement(player.id, "cartographer")
        newly_unlocked.append("cartographer")
    if stats["rooms_explored"] >= 50 and not AchievementRepository.has_achievement(player.id, "room_explorer"):
        unlock_achievement(player.id, "room_explorer")
        newly_unlocked.append("room_explorer")
    if stats["rooms_explored"] >= 500 and not AchievementRepository.has_achievement(player.id, "dungeon_master"):
        unlock_achievement(player.id, "dungeon_master")
        newly_unlocked.append("dungeon_master")
    
    # Quest achievements
    if stats["quests_completed"] >= 1 and not AchievementRepository.has_achievement(player.id, "first_steps"):
        unlock_achievement(player.id, "first_steps")
        newly_unlocked.append("first_steps")
    if stats["quests_completed"] >= 5 and not AchievementRepository.has_achievement(player.id, "quest_novice"):
        unlock_achievement(player.id, "quest_novice")
        newly_unlocked.append("quest_novice")
    if stats["quests_completed"] >= 25 and not AchievementRepository.has_achievement(player.id, "quest_master"):
        unlock_achievement(player.id, "quest_master")
        newly_unlocked.append("quest_master")
    if stats["issues_completed"] >= 10 and not AchievementRepository.has_achievement(player.id, "issue_resolver"):
        unlock_achievement(player.id, "issue_resolver")
        newly_unlocked.append("issue_resolver")
    if stats["prs_completed"] >= 10 and not AchievementRepository.has_achievement(player.id, "pr_champion"):
    ...
    if item_count >= 10 and not AchievementRepository.has_achievement(player.id, "item_collector"):
        unlock_achievement(player.id, "item_collector")
        newly_unlocked.append("item_collector")
    if item_count >= 50 and not AchievementRepository.has_achievement(player.id, "treasure_hunter"):
        unlock_achievement(player.id, "treasure_hunter")
        newly_unlocked.append("treasure_hunter")
    if item_count >= 100 and not AchievementRepository.has_achievement(player.id, "legendary_collector"):
        unlock_achievement(player.id, "legendary_collector")
        newly_unlocked.append("legendary_collector")
    
    # XP achievements
    if stats["total_xp_earned"] >= 10000 and not AchievementRepository.has_achievement(player.id, "xp_hoarder"):
        unlock_achievement(player.id, "xp_hoarder")
        newly_unlocked.append("xp_hoarder")
    if stats["total_xp_earned"] >= 100000 and not AchievementRepository.has_achievement(player.id, "xp_master"):
    ...
    if vibrant_count >= 5 and not AchievementRepository.has_achievement(player.id, "vibrant_explorer"):
    ...
    if undead_count >= 5 and not AchievementRepository.has_achievement(player.id, "undead_hunter"):
    ...
    if star_repos and not AchievementRepository.has_achievement(player.id, "star_gazer"):
    ...
    if mega_star_repos and not AchievementRepository.has_achievement(player.id, "mega_star"):
    ...
    if len(languages) >= 5 and not AchievementRepository.has_achievement(player.id, "language_master"):
    ...
    if max_stat >= 50 and not AchievementRepository.has_achievement(player.id, "stat_booster"):
        unlock_achievement(player.id, "stat_booster")
        newly_unlocked.append("stat_booster")
    if max_stat >= 100 and not AchievementRepository.has_achievement(player.id, "stat_master"):
    ...
    if context:
        ...
        if "enemy" in context and "enemy_level" in context:
            ...
            if enemy_level >= player.level + 10 and not AchievementRepository.has_achievement(player.id, "giant_killer"):
        ...
        if "item_rarity" in context:
            ...
            if rarity in ["rare", "epic", "legendary"] and not AchievementRepository.has_achievement(player.id, "rare_find"):
                unlock_achievement(player.id, "rare_find")
                newly_unlocked.append("rare_find")
            if rarity in ["epic", "legendary"] and not AchievementRepository.has_achievement(player.id, "epic_loot"):
                unlock_achievement(player.id, "epic_loot")
                newly_unlocked.append("epic_loot")
            if rarity == "legendary" and not AchievementRepository.has_achievement(player.id, "legendary_hero"):
                unlock_achievement(player.id, "legendary_hero")
                newly_unlocked.append("legendary_hero")
        
        # Repository Rookie
        if "repository_added" in context and not AchievementRepository.has_achievement(player.id, "repository_rookie"):
