...
class ScrapingThread(QThread):
    ...
    def run(self):
        ...
        try:
            world = build_repo_world(
                self.owner, self.repo, self.scraper, self.progress_callback
            )
...
class MainWindow(QMainWindow):
        ...
        if last_player_id:
            if game_state.load_player(int(last_player_id)):
                self.status_bar.set_status(
                    f"Loaded player: {game_state.current_player.name}"
                )
    ...
    def load_player_dialog(self):
        ...
        if not players:
            QMessageBox.information(
                self, "No Players", "No players found. Create a new game first."
            )
            return
    ...
    def on_scraping_finished(self, world):
        ...
        if world:
            ...
            if game_state.current_player:
                from github_heroes.game.achievements import (
                    ACHIEVEMENTS,
                    check_achievements,
                )
    ...
    def on_start_quest(self, quest_id: int):
        ...
        if enemy:
            ...
            if game_state.current_player:
                ...
                dialog.combat_ended.connect(
                    lambda result, data: self.on_combat_ended(quest_id, result, data)
                )
                dialog.exec()
            else:
                QMessageBox.warning(
                    self, "No Player", "Please create or load a player first."
                )
        else:
    ...
    def on_room_selected(self, room_id: int):
        ...
        if not room.visited:
            ...
            dialog = CombatDialog(
                game_state.current_player, enemy, room.loot_quality, parent=self
            )
            dialog.combat_ended.connect(
                lambda result, data: self.on_room_combat_ended(room_id, result, data)
            )
            dialog.exec()
            ...
            if quest:
                ...
                if game_state.current_player:
                    PlayerStatsRepository.increment_stat(
                        game_state.current_player.id, "quests_completed", 1
                    )
                    if quest.source_type == "issue":
                        PlayerStatsRepository.increment_stat(
                            game_state.current_player.id, "issues_completed", 1
                        )
                    elif quest.source_type == "pr":
                        PlayerStatsRepository.increment_stat(
                            game_state.current_player.id, "prs_completed", 1
                        )
                    ...
                    if newly_unlocked:
                        ...
                        if achievement_names:
                            ...
                            QMessageBox.information(
                                self, "Achievement Unlocked!", ach_msg
                            )
        ...
        if result == "victory":
            ...
            if room:
                if not room.visited and game_state.current_player:
                    PlayerStatsRepository.increment_stat(
                        game_state.current_player.id, "rooms_explored", 1
                    )
                room.visited = True
