...
class QuestBoardView(QWidget):
    ...
    def init_ui(self):
        ...
        self.quests_table.setHorizontalHeaderLabels(
            ["Type", "Title", "Difficulty", "Status", "Actions"]
        )
        self.quests_table.horizontalHeader().setStretchLastSection(True)
        self.quests_table.setSelectionBehavior(
    ...
    def refresh_quests(self, world_id: int):
        ...
        for quest in quests:
            ...
            action_btn.clicked.connect(
                lambda checked, qid=quest.id: self.on_start_quest(qid)
            )
