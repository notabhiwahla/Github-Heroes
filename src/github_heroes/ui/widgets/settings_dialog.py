...
class SettingsDialog(QDialog):
    ...
    def init_ui(self):
        ...
        token_label.setToolTip(
            "Optional GitHub API token for higher rate limits and extended metadata"
        )
        ...
        branch_label.setToolTip(
            "Override automatic branch detection (main/master). Leave empty for auto-detection."
        )
        ...
        self.auto_refresh_checkbox.setToolTip(
            "Automatically refresh repository data when entering a dungeon"
        )
        ...
        reset_db_btn.setToolTip(
            "WARNING: This will delete all game data including players, worlds, and items!"
        )
    ...
    def save_settings(self):
        ...
        db.set_setting(
            "auto_refresh",
            "true" if self.auto_refresh_checkbox.isChecked() else "false",
        )
            ...
            if reply2 == QMessageBox.StandardButton.Yes:
                try:
                    ...
                    QMessageBox.critical(
                        self, "Error", f"Failed to reset database:\n{str(e)}"
                    )
